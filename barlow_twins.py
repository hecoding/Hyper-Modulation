...
def compute_contrastive(discriminator, images, barlow_twins, transform, c, step, alpha, weighting=0.01):
    ...
    feats1 = hierarchical_feats1[-2].squeeze(-1).squeeze(-1)
    feats2 = hierarchical_feats2[-2].squeeze(-1).squeeze(-1)

    # hack to compute the cross-corr matrix among gpus on DP
    z1, z2 = barlow_twins(feats1, feats2)  # gather zs from all gpus
    final_loss = barlow_twins.module.compute_final_loss(z1, z2)  # .module = out of DP
