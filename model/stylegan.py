...
class Generator(nn.Module):
    ...
    def forward(self, style, noise, step=0, alpha=-1, mixing_range=(-1, -1), return_hierarchical=False):
        out = noise[0]
        hierarchical_out = []
...
class StyledGenerator(nn.Module):
    def __init__(self, code_dim=512, n_mlp=8, c_dim=0):
        super().__init__()
        ...
        for i in range(n_mlp):
            layers.append(EqualLinear(code_dim + (code_dim if i == 0 and c_dim > 0 else 0), code_dim))
            layers.append(nn.LeakyReLU(0.2))

        self.style = nn.Sequential(*layers)
        self.embed = EqualLinear(c_dim, code_dim) if c_dim > 0 else None
    ...
    def forward(
        ...
        for i in input:
            if c is not None:
                i = torch.cat([i, self.embed(c)], dim=1)
...
class Discriminator(nn.Module):
    def __init__(self, fused=True, from_rgb_activate=False, num_labels=0):
        super().__init__()
        ...
        self.linear = EqualLinear(512, 1 if num_labels == 0 else num_labels)

    def forward(self, input, step=0, alpha=-1, c=None):
